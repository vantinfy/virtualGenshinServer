...
func (p *ModPool) UpPoolDrawWithCheck(player *Player, times int, opts ...Options) {
	cards := map[int]int{}
	switches := p.InitStatistic(opts...) // 开关 记录统计哪些信息
	for i := 0; i < times; i++ {
		p.UpPoolInfo.Statistics.HistoryCnt++ // 历史抽卡数+1
		...
		fiveInfos, fourInfos := player.ModRole.GetRolesInfoForCheck()

		card := csvs.GetRandDropRecursionForCheck(dropGroup, fiveInfos, fourInfos)
		if card != nil {
			// 考虑了保底歪了武器
			role := csvs.GetRoleConfig(card.Result)
			if role != nil { // 出了角色
...
func (p *ModPool) UpPoolDrawWithCheckAvg(player *Player, times int, opts ...Options) {
	cards := map[int]int{}
	switches := p.InitStatistic(opts...) // 开关 记录统计哪些信息
	for i := 0; i < times; i++ {
		p.UpPoolInfo.Statistics.HistoryCnt++ // 历史抽卡数+1
		...
		// 获取已有角色
		fiveInfos, fourInfos := player.ModRole.GetRolesInfoForCheck()
		card := csvs.GetRandDropRecursionForCheckAvg(dropGroup, fiveInfos, fourInfos)
		if card != nil {
			// 考虑了保底歪了武器
			role := csvs.GetRoleConfig(card.Result)
			if role != nil { // 出了角色
